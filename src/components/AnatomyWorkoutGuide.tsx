...
export function AnatomyWorkoutGuide() {
  ...
  const [renderMode, setRenderMode] = useState<'realistic' | 'anatomy3d'>('realistic');
  ...
  return (
    <div className="w-full max-w-5xl mx-auto flex flex-col items-center justify-center text-center animate-in fade-in slide-in-from-bottom-4 duration-700 pb-20 px-3 sm:px-4 md:px-6">
      ...
      <div className="w-full max-w-3xl bg-gradient-to-b from-[#0e1626] via-[#09101d] to-[#040810] rounded-[32px] sm:rounded-[40px] shadow-2xl border border-emerald-500/20 overflow-hidden relative flex flex-col items-center justify-center p-3 sm:p-5">
        ...
        {/* Top Floating Badge, Mode Switcher & Angle Selectors */}
        <div className="w-full flex flex-wrap items-center justify-between gap-2 px-3 pt-2 z-10">
          <div className="flex items-center gap-2">
            ...
            {/* Realistic Avatar vs 3D Anatomy Mesh Mode Toggle */}
            <div className="flex items-center gap-1 p-1 bg-slate-950/80 backdrop-blur-md rounded-full border border-emerald-500/40 text-xs">
              <button
                onClick={() => {
                  setRenderMode('realistic');
                  playSfx('tap');
                }}
                className={`px-3 py-1 rounded-full text-[11px] font-bold cursor-pointer transition-all flex items-center gap-1.5 ${
        ...
        {/* STAGE VIEWPORT: REALISTIC AVATAR OR 3D MESH */}
        <div className="w-full h-[400px] sm:h-[480px] relative flex items-center justify-center overflow-hidden">
          {renderMode === 'realistic' ? (
            /* Realistic Athletic Avatar Stage View */
            <div className="w-full h-full relative flex items-center justify-center select-none">
              {/* Background ambient lighting effects */}
              <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_center,_var(--tw-gradient-stops))] from-blue-950/40 via-transparent to-black pointer-events-none" />
              <div className="absolute top-1/4 -left-12 w-48 h-48 bg-blue-500/10 rounded-full blur-3xl pointer-events-none" />
              <div className="absolute top-1/4 -right-12 w-48 h-48 bg-emerald-500/10 rounded-full blur-3xl pointer-events-none" />

              {/* Realistic Avatar Image with pulse animation matching playback */}
              <div className="relative w-full h-full flex items-center justify-center p-2">
                ...
                >
                  ...
                  <div className="absolute top-8 left-4 sm:left-8 bg-slate-950/85 backdrop-blur-md border border-emerald-400/40 px-3 py-1.5 rounded-xl shadow-lg flex items-center gap-2">
                    <span className="w-2.5 h-2.5 rounded-full bg-emerald-400 animate-pulse" />
                    <span className="text-[11px] font-bold text-white tracking-wide">{currentExercise.targetMuscle}</span>
                  </div>

                  <div className="absolute top-8 right-4 sm:right-8 bg-slate-950/85 backdrop-blur-md border border-blue-400/40 px-3 py-1.5 rounded-xl shadow-lg flex items-center gap-2">
                    <Dumbbell className="w-3.5 h-3.5 text-blue-400" />
                    <span className="text-[11px] font-bold text-white tracking-wide">Halteres 10kg</span>
                  </div>
                </motion.div>
              </div>

              {/* Exercise Name Overlay at the bottom of the 3D Stage (Matching user reference) */}
              <div className="absolute bottom-4 left-0 right-0 flex flex-col items-center justify-center pointer-events-none px-4 z-10">
                <div className="bg-slate-950/85 backdrop-blur-md px-6 py-2.5 rounded-2xl border border-emerald-500/30 shadow-2xl flex flex-col items-center">
                  ...
                  <p className="text-emerald-400 text-xs sm:text-sm font-bold tracking-wide mt-0.5">
            ...
            /* 3D Three.js Canvas Stage with Lazy Loading and Biometric Scan Fallback */
            <Suspense
              fallback={
                <Static3DAvatarPlaceholder
                  title={currentExercise.name}
                  subtitle="Inicializando malha biomecânica e acelerador WebGL..."
                  heightClass="h-full min-h-[420px]"
                />
              }
            >
              <Anatomy3DCanvasStage
