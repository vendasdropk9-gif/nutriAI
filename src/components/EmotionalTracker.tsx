...
export function EmotionalTracker({
  ...
  const [faceCameraStream, setFaceCameraStream] = useState<MediaStream | null>(
    null,
  );
  ...
  const startFaceCamera = async () => {
    ...
    try {
      ...
      try {
        ...
        console.warn(
          "Retrying with simple video constraints due to:",
          firstErr,
        );
      ...
      if (
        ...
        userFriendlyMsg =
          "Câmera de vídeo física não encontrada ou indisponível neste dispositivo. Por favor, envie uma foto de sua expressão facial utilizando a seleção manual de fotos abaixo.";
      }
    ...
    try {
      ...
      if (data) {
        ...
        if (data.detectedMood) {
          ...
          const solvedMood = validMoods.includes(matchedMood)
            ? matchedMood
            : "neutro";
        ...
        alert(
          "Não foi possível realizar a análise facial. Tente tirar a foto com melhor iluminação e de forma frontal.",
        );
      }
  ...
  return (
    <div className="w-full overflow-x-hidden max-w-4xl mx-auto px-2 sm:px-0 space-y-12 animate-in fade-in slide-in-from-bottom-4 duration-700 pb-20">
      <div className="text-center space-y-4 px-2">
        ...
        <p className="font-sans text-slate-500 dark:text-slate-400 max-w-xl mx-auto text-base sm:text-lg leading-relaxed">
          Entenda como suas emoções influenciam sua alimentação e descubra
          padrões de fome emocional com ajuda da IA.
        </p>
      </div>
      ...
      <div className="clay-card p-4 sm:p-6 md:p-8 mx-2 sm:mx-0">
        <div className="space-y-8">
          <div className="text-center px-2">
            <h3 className="font-serif text-xl sm:text-2xl text-slate-800 dark:text-slate-100 font-medium">
              Como você está se sentindo agora?
            </h3>
          </div>

          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-3 sm:gap-4 px-2 sm:px-0">
            {Object.entries(moodIcons).map(([key, { icon, color }]) => (
              <button
                key={key}
                onClick={() => setMood(key)}
                className={`flex flex-col items-center justify-center p-3 sm:p-4 rounded-2xl border shadow-sm transition-all hover:scale-105 active:scale-95 min-w-0 ${mood === key ? "bg-emerald-500 text-white border-emerald-500 shadow-emerald-500/20" : color}`}
              >
                {React.cloneElement(icon, {
                  className: "w-5 h-5 sm:w-6 sm:h-6 shrink-0",
                })}
                <span className="text-[10px] sm:text-xs font-bold uppercase mt-2 truncate w-full text-center">
                  {key}
                </span>
              </button>
            ))}
            ...
            <div className="space-y-6 animate-in slide-in-from-top-4 duration-300">
              <div className="space-y-3">
                <label className="block text-sm font-bold text-slate-400 uppercase tracking-widest">
                  Algum gatilho específico? (Opcional)
                </label>
                <input
                  type="text"
                  value={trigger}
        ...
        <div className="flex items-start gap-3 sm:gap-4">
          ...
          <div className="min-w-0 flex-1">
            ...
            <p className="font-sans text-slate-500 dark:text-slate-400 text-xs sm:text-sm leading-relaxed mt-1">
              Uma funcionalidade 100% opcional e amigável. Ative voluntariamente
              a sua câmera para analisar expressões sutis de humor ou ansiedade,
              ajudando a compor sugestões nutritivas e chás para o final do seu
              dia.
            </p>
          </div>
        </div>
            ...
            <div className="space-y-6 bg-slate-50 dark:bg-slate-800/40 border border-slate-105 dark:border-slate-800/80 p-4 sm:p-5 rounded-2xl">
              <div className="flex gap-3 bg-emerald-50 dark:bg-emerald-950/20 border border-emerald-100/40 dark:border-emerald-900/30 p-3 sm:p-4 rounded-xl items-start">
                ...
                <div className="text-xs sm:text-sm text-emerald-800 dark:text-emerald-400 leading-relaxed min-w-0 flex-1">
                  <strong>Privacidade em Primeiro Lugar:</strong> O NutriAI
                  analisa padrões visuais de forma estritamente local e segura,
                  **sem nunca salvar, persistir ou monitorar continuamente**
                  suas imagens. O processamento é descartado na hora,
                  respeitando totalmente você.
                </div>
              </div>
              ...
              <div className="space-y-3">
                <label className="flex items-start gap-3 cursor-pointer group min-w-0">
                  ...
                  <span className="text-xs sm:text-sm text-slate-600 dark:text-slate-300 leading-relaxed select-none group-hover:text-slate-800 dark:group-hover:text-white min-w-0 flex-1">
                    Compreendo que a câmera frontal será usada apenas
                    temporariamente para identificar nuances sutis de cansaço ou
                    ansiedade focando no meu equilíbrio de bem-estar.
                  </span>
                </label>
                ...
                <label className="flex items-start gap-3 cursor-pointer group min-w-0">
                  ...
                  />
                  <span className="text-xs sm:text-sm text-slate-600 dark:text-slate-300 leading-relaxed select-none group-hover:text-slate-800 dark:group-hover:text-white min-w-0 flex-1">
                    Consinto ativamente em ativar a câmera para obter minhas
                    sugestões personalizadas de alimentação e práticas de
                    descanso de hoje.
                  </span>
                </label>
              </div>
      ...
      <div className="grid md:grid-cols-2 gap-8 px-2 sm:px-0">
        ...
        <div className="bg-white dark:bg-slate-800 p-6 sm:p-8 rounded-[32px] border border-slate-105 dark:border-slate-800/80 text-slate-800 dark:text-white shadow-xl flex flex-col justify-between transition-colors duration-300 min-w-0">
          <div className="space-y-4 min-w-0">
            ...
            <p className="text-sm sm:text-base text-slate-600 dark:text-slate-300 leading-relaxed font-sans break-words">
              Minha IA identifica se você está comendo por ansiedade ou tédio ao
              cruzar seus horários e sentimentos.
            </p>
          </div>
          <button
            onClick={handleAnalyze}
            disabled={isAnalyzing}
            className="mt-8 bg-slate-900 hover:bg-slate-800 dark:bg-white dark:text-slate-900 text-white py-4 px-4 sm:px-8 rounded-2xl font-bold shadow-md hover:shadow-lg transition-all flex items-center justify-center gap-2 sm:gap-3 disabled:opacity-50 cursor-pointer w-full min-w-0"
          >
        ...
        {/* History */}
        <div className="clay-card p-6 sm:p-8 min-w-0">
          <div className="flex items-center gap-3 mb-6 min-w-0">
            <History className="w-6 h-6 text-slate-400 shrink-0" />
            <h3 className="font-serif text-xl sm:text-2xl text-slate-800 dark:text-slate-100 font-medium truncate">
              Últimos Registros
            </h3>
          </div>
            ...
            {logs.length === 0 ? (
              ...
              logs
                ...
                .map((log) => (
                  <div
                    key={log.id}
                    className="flex items-center gap-3 p-3 sm:p-4 rounded-2xl bg-slate-50 dark:bg-slate-800/40 border border-slate-100 dark:border-slate-800/80 shadow-sm min-w-0"
                  >
                    <div
                      className={`p-2 rounded-lg shrink-0 ${moodIcons[log.mood]?.color || "bg-slate-100 text-slate-500"}`}
                    >
                      {moodIcons[log.mood]?.icon || <Meh className="w-4 h-4" />}
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className="font-bold text-slate-700 dark:text-slate-200 capitalize text-sm truncate">
                        {log.mood}
                      </p>
                      {log.trigger && (
        ...
        <div className="animate-in fade-in slide-in-from-bottom-8 duration-700 space-y-8">
          ...
          <div className="grid md:grid-cols-2 gap-8">
            <div className="clay-card p-6 space-y-4">
              ...
              <p className="text-xl text-slate-700 dark:text-slate-200 font-serif leading-relaxed">
                {analysis.insight}
              </p>
            </div>
            <div className="bg-emerald-50/50 dark:bg-emerald-900/20 p-8 rounded-[32px] clay-card border border-emerald-100/50 dark:border-emerald-800/30 space-y-4">
              <h4 className="text-xs font-bold text-emerald-600 dark:text-emerald-400 uppercase tracking-widest text-emerald-500">
                Sugestão Prática:
              </h4>
              <p className="text-xl text-emerald-800 dark:text-emerald-300 font-serif leading-relaxed">
                {analysis.suggestion}
              </p>
            </div>
          </div>
