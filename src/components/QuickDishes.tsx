            ...
            {dishes.map((dish, idx) => {
              ...
              return (
                <motion.div
                  ...
                  className="relative flex flex-col bg-white dark:bg-[#131920] rounded-[28px] border border-slate-200/80 dark:border-slate-800/80 shadow-md hover:shadow-xl hover:border-emerald-500/40 transition-all duration-300 overflow-hidden group"
                >
                  {/* Photo Header */}
                  ...
                  {/* IN-CARD RECIPE DIRECT OVERLAY - Appears right in front of the generated dish card */}
                  <AnimatePresence>
                    {isCardRecipeOpen && (
                      <motion.div
                        ...
                        className="absolute inset-0 z-30 bg-white/95 dark:bg-[#11171E]/95 backdrop-blur-md flex flex-col p-4 sm:p-5 overflow-y-auto"
                      >
                        {/* Overlay Header */}
                        ...
                        <div className="space-y-4 pt-3 flex-1">
                          ...
                          <div className="flex flex-wrap items-center gap-2 text-[11px] font-semibold text-slate-600 dark:text-slate-300">
                            <span className="px-2 py-1 rounded-lg bg-orange-500/10 text-orange-600 dark:text-orange-400 flex items-center gap-1">
                              <Flame className="w-3 h-3" /> {dish.nutrition.calories} kcal
                            </span>
                            <span className="px-2 py-1 rounded-lg bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 flex items-center gap-1">
                              <Sparkles className="w-3 h-3" /> {dish.nutrition.protein}g prot
                            </span>
                            <span className="px-2 py-1 rounded-lg bg-slate-100 dark:bg-slate-800 flex items-center gap-1">
                              <Clock className="w-3 h-3 text-emerald-500" /> {dish.prepTime}
                            </span>
                          </div>
                          ...
                          <div className="space-y-2">
                            ...
                            <div className="space-y-1.5 max-h-40 overflow-y-auto pr-1">
                              {dish.ingredients.map((ing, iIdx) => {
                                const ingKey = `${dish.id}-card-ing-${iIdx}`;
                                ...
                                return (
                                  <button
                                    ...
                                    className={`w-full flex items-center gap-2 p-2 rounded-xl text-left text-xs transition-all border ${
                                      isChecked
                                        ? 'bg-emerald-500/10 border-emerald-500/30 text-slate-400 line-through'
                                        : 'bg-slate-50 dark:bg-[#18212C] border-slate-100 dark:border-slate-800 text-slate-800 dark:text-slate-200'
                                    }`}
                                  >
                          ...
                          <div className="space-y-2">
                            ...
                            <div className="space-y-2">
                              {dish.instructions.map((step, sIdx) => (
                                <div key={sIdx} className="flex items-start gap-2 p-2 rounded-xl bg-slate-50 dark:bg-[#18212C] border border-slate-100 dark:border-slate-800 text-xs text-slate-700 dark:text-slate-300">
                                  <span className="shrink-0 w-4 h-4 rounded-full bg-emerald-500 text-white font-bold text-[10px] flex items-center justify-center mt-0.5">
                                    {sIdx + 1}
                                  </span>
